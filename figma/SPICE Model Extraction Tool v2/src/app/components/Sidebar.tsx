import {
  ...
  CheckCircle2, Download, Settings, Zap, ChevronRight, FolderOpen, Layers
} from "lucide-react";

export type NavSection =
  | "dashboard" | "data" | "curve" | "model"
  | "workbench" | "fitting" | "validate" | "export" | "settings";
...
const navItems: { id: NavSection; label: string; icon: React.ComponentType<{ size?: number; color?: string }> }[] = [
  ...
  { id: "fitting",   label: "TransferFit", icon: Sliders },
  ...
  return (
    <div style={{
      ...
      <nav style={{ flex: 1, padding: "8px 8px", overflowY: "auto" }}>
        ...
        {navItems.slice(0, 7).map(item => <NavItem key={item.id} item={item} active={activeNav === item.id} onClick={() => onNavChange(item.id)} />)}
        ...
        {navItems.slice(7).map(item => <NavItem key={item.id} item={item} active={activeNav === item.id} onClick={() => onNavChange(item.id)} />)}
      </nav>
